...
#==#
if [[ $# -ne 7 ]]; then
    echo "Usage: geogit_sync.sh <direction> <user> <password> <repo> <remote> <authorname> <authoremail>"
    ...
    #==#
    cd $DIR
    $PYTHON manage.py geogit-sync --direction $DIRECTION --username $USER --password $PASSWORD --url "$GEONODE_LOCAL$CTX_GEOGIT$REPO/" --remote $REMOTE --authorname $AUTHORNAME --authoremail $AUTHOREMAIL
fi
